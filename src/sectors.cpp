...
#include "math.h"
#define SCALE 200
bool IsOnCamera(Wall wall, float min_angle, float max_angle) {
  float v0angle = atan2(wall.v0.y, wall.v0.x);
  float v1angle = atan2(wall.v1.y, wall.v1.x);
  return v0angle >= min_angle && v1angle >= min_angle && v0angle <= max_angle &&
         v1angle <= max_angle;
}
bool IsOnCameraPoint(v2f point, float min_angle,float max_angle){
...
void RenderSectors(SDL_Renderer *renderer, Sectors sectors, Player player) {
  ...
  for (Sector sector : sectors) {
    for (Wall wall : sector.walls) {
      float min_angle = atan2(WIDTH /(2*SCALE),1);   
      float max_angle =atan2(WIDTH /(2*SCALE),-1);
      cout<<min_angle<<" "<<max_angle <<endl;
      Wall w = RotateAndMoveWall(wall, player);

      if (!IsOnCamera(w, min_angle, max_angle)) continue;
      v2f pf0;
      v2f pf1;
      v2f pc0;
      v2f pc1;
      ...
      // ceiling
      pc0.x = x0;
      pc0.y = ((sector.zFloor + sector.zCeiling - height_camera) / y0) * SCALE +
              HEIGHT / 2;

      pc1.x = x1;
      pc1.y = ((sector.zFloor + sector.zCeiling - height_camera) / y1) * SCALE +
              HEIGHT / 2;
